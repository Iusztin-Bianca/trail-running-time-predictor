...
class DataIngestionPipeline:
    ...
    def run(self, after: Optional[int] = None) -> pd.DataFrame:
        ...
        if not df.empty:
            ...
            logger.info(f"\nSegment distance statistics:")
            logger.info(f"  - Mean: {df['segment_distance_m'].mean():.1f} m")
            logger.info(f"  - Median: {df['segment_distance_m'].median():.1f} m")
            logger.info(f"  - Min: {df['segment_distance_m'].min():.1f} m")
            logger.info(f"  - Max: {df['segment_distance_m'].max():.1f} m")
            logger.info(f"\nSegment time statistics:")
            logger.info(f"  - Mean: {df['segment_time_sec'].mean():.1f} m")
            logger.info(f"  - Median: {df['segment_time_sec'].median():.1f} m")
            logger.info(f"  - Min: {df['segment_time_sec'].min():.1f} m")
            logger.info(f"  - Max: {df['segment_time_sec'].max():.1f} m")
            logger.info("=" * 80)
